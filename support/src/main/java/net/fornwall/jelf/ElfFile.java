...
public final class ElfFile {
	...
	public ArrayList<ElfSection> getSections() throws ElfException, IOException{
		ArrayList<ElfSection> sections = new ArrayList<>();
		for (int i=0;i<num_sh;i++){
			ElfSection section = sectionHeaders[i].getValue();
	...
	public ArrayList<ElfSegment> getLoadSegments() throws IOException {
		ArrayList<ElfSegment> segments = new ArrayList<>();
		for (int i=0;i<num_ph;i++){
			ElfSegment segment = programHeaders[i].getValue();
			if (segment.type == ElfSegment.PT_LOAD){
	...
	public ElfSegment getDynamicSegment() throws ElfException, IOException{
		return findSegmentByType(ElfSegment.PT_DYNAMIC);
	}

	private ElfSegment findSegmentByType(int type) throws ElfException, IOException {
